...
class Movenet(object):
    ...
    def init_crop_region(self, image_height: int, image_width: int) -> Dict[(str, float)]:
        ...
        return {'y_min': y_min, 'x_min': x_min, 'y_max': y_min + box_height, 'x_max': x_min + box_width,
    ...
    def _determine_torso_and_body_range(self, keypoints: np.ndarray, target_keypoints: Dict[(str, float)],
                                        center_y: float, center_x: float) -> List[float]:
    ...
    def _determine_crop_region(self, keypoints: np.ndarray, image_height: int, image_width: int) -> Dict[(str, float)]:
        ...
        if self._torso_visible(keypoints):
            ...
            (max_torso_yrange, max_torso_xrange, max_body_yrange,
             max_body_xrange) = self._determine_torso_and_body_range(keypoints, target_keypoints, center_y, center_x)

            crop_length_half = np.amax([max_torso_xrange * Movenet._TORSO_EXPANSION_RATIO,
            ...
            return {
                'y_min': crop_corner[0] / image_height, 'x_min': crop_corner[1] / image_width,
    ...
    def _crop_and_resize(self, image: np.ndarray, crop_region: Dict[(str, float)], crop_size: (int, int)) -> np.ndarray:
        ...
        y_min, x_min, y_max, x_max = [crop_region['y_min'], crop_region['x_min'],
        ...
        output_image = cv2.copyMakeBorder(output_image, padding_top, padding_bottom, padding_left, padding_right,
                                          cv2.BORDER_CONSTANT)
