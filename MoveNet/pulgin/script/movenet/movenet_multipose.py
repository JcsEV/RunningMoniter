...
class MoveNetMultiPose(object):
    ...
    def __init__(self, model_name: str, tracker_type: str = 'bounding_box', input_size: int = 256) -> None:
        ...
        if tracker_type == 'keypoint':
            ...
            print('ERROR: Tracker type {0} not supported. No tracker will be used.'.format(tracker_type))
            self._tracker = None

    def detect(self, input_image: np.ndarray, detection_threshold: float = 0.11) -> List[Person]:
        ...
        if is_dynamic_shape_model:
            resized_image, _ = utils.keep_aspect_ratio_resizer(input_image, self._input_size)
            input_tensor = np.expand_dims(resized_image, axis=0)
            self._interpreter.resize_tensor_input(self._input_details[0]['index'], input_tensor.shape, strict=True)
        else:
        ...
        self._interpreter.set_tensor(self._input_details[0]['index'], input_tensor.astype(self._input_type))
        self._interpreter.invoke()

        # Get the model output
        model_output = self._interpreter.get_tensor(self._output_details[0]['index'])

        image_height, image_width, _ = input_image.shape
        return self._postprocess(model_output, image_height, image_width, detection_threshold)

    def _postprocess(self, keypoints_with_scores: np.ndarray, image_height: int, image_width: int,
        ...
        for idx in range(num_instances):
            ...
            for i in range(scores.shape[0]):
                keypoints.append(
                    KeyPoint(BodyPart(i), Point(int(kpts_x[i] * image_width), int(kpts_y[i] * image_height)), scores[i]))

            # Calculate the bounding box
            rect = [keypoints_with_scores[0, idx, 51], keypoints_with_scores[0, idx, 52],
                    keypoints_with_scores[0, idx, 53], keypoints_with_scores[0, idx, 54]]
            bounding_box = Rectangle(Point(int(rect[1] * image_width), int(rect[0] * image_height)),
